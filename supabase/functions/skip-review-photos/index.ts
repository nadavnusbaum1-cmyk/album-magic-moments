// Host-only: mark review photos as skipped so they don't reappear in the Review tab.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error } = await supabase.from("photos").update({ review_skipped: true }).in("id", photoIds);
    if (error) throw error;
    return json({ ok: true, skipped: photoIds.length });
  } catch (e) {
